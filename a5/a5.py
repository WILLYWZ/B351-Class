...
def entropy(classifications):
    ...
    label, count = unique(classifications)
    entropy = 0
    for i in count:
        entropy += -(i/size * math.log2(i/size))
        i += 1
    return entropy
...
def information_gain(parent_classifications, classifications_by_val, val_freqs):
    size = len(parent_classifications)
    E = entropy(parent_classifications)   
    sum = 0
    for value, key in classifications_by_val.items():
        sum += entropy(key)* len(key)/size 
    return E - sum
    ...
    def classify(self, point):
        if self.classification is not None:
            ...
            if val in self.children.keys():
                node = self.children.get(val)
            else:
                node = self.children.get(OTHER)
        return node.classify(point)
...
class KNN_Classifier:
    ...
    def euclidean_distance(self, point1, point2):
        ...
        for i in range(len(point1)):
            answer += math.pow((point1[i]-point2[i]),2)
    ...
    def classify(self, point, sample_points, sample_labels):
        lst1 = []
        for pnt, label in zip(sample_points, sample_labels):
            a = self.euclidean_distance(point, pnt)
            lst1.append((a,label))
        lst1.sort()
