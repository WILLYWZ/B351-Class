...
def fringe_expansion(current_state, fringe):
    up = current_state.board.slide_blank((0,1))
    down = current_state.board.slide_blank((0,-1))
    left = current_state.board.slide_blank((1,0))
    right = current_state.board.slide_blank((-1,0))
...
def my_new_heuristic(current_board, goal_board):
    sum = 0
    for i in range(1, 9):
        current_row, current_col = current_board.find_element(i)
        goal_row, goal_col = goal_board.find_element(i)
        row = abs(current_row - goal_row)
        col = abs(current_col - goal_col)
        flag = 3.9999999 *0.5
        sum += abs(row -col) + min(row, col) * flag
    return sum
...
def informed_expansion(current_state, fringe, f_function):
    ...
    for move in movement_list:
        if move is not None:   
            heapq.heappush(fringe, State.State(move, current_state, current_state.depth +1, f_function(move, current_state.depth +1)))
